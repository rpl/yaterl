...
generate_files(ProjectName, ProjectType) ->
    ...
    {ok, RebarConfigSrc} = rebar_config_dtl:render(TplVars),
    BaseFileName = ProjectName++"/src/"++ProjectName,
    io:format("Generating file: ~s.app.src~n", [ProjectName]),
    ok = file:write_file(BaseFileName ++ ".app.src", AppSrc),
    io:format("Generating file: ~s.erl~n", [ProjectName]),
    ...
    io:format("Generating file: rebar.config~n"),
    ok = file:write_file(ProjectName++"/rebar.config", RebarConfigSrc),
    ...
    io:format("Generating file: yaterl-devtool~n"),
